...
def cmake_build(Map conf=[:]){
    ...
    def build_cmd = conf.get("build_cmd", "LLVM_PATH=/opt/rocm/llvm ${build_envs} dumb-init make -j\$(( \$(nproc) < 32 ? \$(nproc) : 32 )) ${config_targets}")
...
def cmake_fin_build_cmd(prefixpath){
    ...
    def fin_cmd = """
            ...
            dumb-init make -j\$(( \$(nproc) < 32 ? \$(nproc) : 32 )) ${config_targets}
    """
    return fin_cmd
}
...
pipeline {
    ...
    stages{
        stage('Build Docker and Packages'){
            when {
        ...
        stage("Full Tests") {
            ...
            parallel{
                stage('Int8|FP16|FP32 HIP Debug All Vega20') {
                    when {
                        beforeAgent true
                        expression { params.TARGET_VEGA20 && (params.DATATYPE_INT8 || params.DATATYPE_FP16 || params.DATATYPE_FP32)}
                    }
                    options {
                        retry(2)
                    }
                    agent{ label rocmnode("vega20") }
                    steps{
                        buildHipClangJobAndReboot( setup_flags: Int8_flags + Full_test, build_type: 'debug')
                        buildHipClangJobAndReboot( setup_flags: Full_test + Fp16_flags, build_type: 'debug')
                        buildHipClangJobAndReboot( setup_flags: Full_test, build_type: 'debug')
                    }
                }
